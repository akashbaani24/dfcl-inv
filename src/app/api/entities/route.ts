  ...
  try {
    ...
    try {
      entities = await db.entity.findMany({
        orderBy: { name: 'asc' },
        select: { id: true, name: true, description: true, entityType: true, shortCode: true, logo: true, address: true, phone: true, parentEntityId: true, createdAt: true, updatedAt: true },
      });
    } catch (dbErr) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // ★ Prevent circular reference: parentEntityId can't be the same as the new entity's id
    //   (For POST this is always fine since the entity doesn't exist yet, but we validate anyway.)

    const entity = await db.entity.create({
      data: { name, description: description || null, entityType: entityType || 'outlet', shortCode: shortCode || null, logo: logo || null, address: address || null, phone: phone || null, parentEntityId: parentEntityId || null },
    });
