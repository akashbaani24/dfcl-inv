...
if [ -d ".next/standalone" ]; then
    ...
    # 删除 standalone 中携带的 dev 环境 .env，避免覆盖生产环境由 start.sh 注入的 DATABASE_URL
    # Next.js standalone 会把项目的 .env 复制进来，里面的 dev 路径在生产容器中不存在，
    # 会导致 Prisma 打不开数据库 → 健康检查失败 → 部署报错。
    if [ -f "$BUILD_DIR/next-service-dist/.env" ]; then
        echo "  - 移除 standalone 中的 dev .env（生产环境由 start.sh 注入 DATABASE_URL）"
